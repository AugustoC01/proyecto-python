...
class Aplicacion():
	def __init__(self):
		...
		self.menu_bbdd.add_command(label = "Conectar", command = lambda: self.conectar_base(0))
	...
	def ventana_mensaje(self, salir, mensaje):
		if salir:
			...
			mb.showinfo("Base de datos", mensaje)

	#SE CONECTA A LA BASE Y GUARDA LOS DATOS
	def conectar_base(self, opcion):
		if (opcion == 0):
	...
	#REVISA SI HAY DATOS VACIOS
	def revisa_datos(self, lista):
		vacio = False
		for dato in lista:
			if (dato == ""):
				vacio = True
	...
	def crear_entrada(self):
		...
		if self.conectado:
			if (not self.revisa_datos(lista_datos)):
				...
				self.conectar_base(1)
			else:
				self.ventana_mensaje(False, "Error, hay campos vacios")
		else:
			self.ventana_mensaje(False, "Base de datos desconectada")
	...
	def leer_entrada(self):
		if self.conectado:
			try:
				...
				self.ventana_mensaje(False, "Ese registro no existe, ingrese un numero valido")
		else:
			self.ventana_mensaje(False, "Base de datos desconectada")
	...
	def actualizar_entrada(self):
		if self.conectado:
			...
			if (not self.revisa_datos(lista_nuevos)):
				self.cursor.execute('''UPDATE registro_personas SET NOMBRE=?, CLAVE=?, APELLIDO=?, DIRECCION=?, COMENTARIOS=? 
					 				   WHERE ID=''' + str(self.id.get()), lista_nuevos)
